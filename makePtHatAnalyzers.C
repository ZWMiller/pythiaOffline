/*
// This creates a directory ("folder" input option) and then generates all of the 
// required elements to run the online ptHatTemplates Monte Carlo. 
// www.github.com//zaaglamir/pythiaNPEh contains the code for the online portion. 
// The online portion requires a card (to feed options to Pythia), a script (to set the environment 
// and run the .cpp that generates the events using TPythia), and a .job file which is 
// used by condor (RCF) to submit many runs at once. 
//
// All required elements are created and put in an aptly named directory underneat/h the user
// specified "folder" name. 
//
// Then use scp to move to RCF for use.
//
// Z. Miller - 8/27/15
*/
...
Int_t makeScripts(Int_t mode, Int_t pthatmin, Int_t pthatmax, Int_t maxRuns, const char* folder)
{
  ...
  char fName[100];
  char temp[200];
  for(Int_t ii=0;ii<maxRuns;ii++) // Loop to create the file for each run
    {
      // create the file, naming convention has ptHat range and runnum
      if(mode == 1)
	...
	sprintf(fName, "%s/script/run_B%i_%i_%i.csh",folder, pthatmin,pthatmax,ii);
      ...
      outFile << "#!/bin/csh" << endl << endl;                        // specify cShell
      outFile << "source /star/u/zamiller/.cshrc" << endl;            // do csh login, sets $PATH to find PDFs and Pythia install
      outFile << "cd /star/u/zbtang/myTools/root/" << endl;           // Use this version or root with TPythia
      outFile << "source bin/thisroot.csh" << endl;                   // log in to root
      outFile << "cd /star/u/zamiller/simu/ptHatTemplates" << endl;   // go to my folder
      if(mode==1)
	...
	sprintf(temp, "./NPEHDelPhiCorr cards/NpeB%i_%i_%i.cmnd output/npeB%i_%i/NpeBHcorr%i_%i_%i.root B",pthatmin,pthatmax,ii,pthatmin,pthatmax,pthatmin,pthatmax,ii);
      outFile << temp << endl;                                        // Write the execution command. Options: card, output fileName, histogram Label
      outFile.close();
      }
  return 1;
}
...
Int_t makeJobs(Int_t mode, Int_t pthatmin, Int_t pthatmax, Int_t maxRuns, const char* folder)
{
  ...
  // This part is the same for all .job files. Only change the Inital Dir if new analysis directory created
  ...
  // For each run and ptHat bin, tell it what to execute and where to put log files
  if(mode == 1)
...
{
  // make cards, naming convention ptHatRange and Runnum
  for(Int_t run=0; run < numRuns; run++){
    ...
    // All of this stays the same for every ptbin, run, and template choice. Big block is ugly, but necessary.
    ...
    // These are the options that change.
    // turn on ccbar or bbar and turn off the opposite
    if(mode == 1)
      {
